...
- ## **As a new user:**
  ...
  - The 'about' section contains information about Walker's company.
...
- ## **As a returning user:**
  ...
  - After logging in to the user profile, an icon appears in the menu that, when pressed, allows the user to log out.
  ...
  - In the user's profile is an 'edit account' option, and there is the option for 'delete account'.
  ...
  - Search form allows users to find pet sitters in their area.
  - The form contains essential information that will allow the users to find the right person for the care of their dog.

  ![search form](/documents/search.png)

  - The results are presented in the form of a list of pet sitter cards which, after clicking on it, lead to a pet sitter profile.

  ![results](/documents/success-search.png)

  - If the user cannot find anyone, an appropriate message appears on the screen.
  ...
  - User can write a review about the pet sitter in the pet sitter profile part.
    ...
    - There is also a text area under the stars where the user can write their opinion.

    ![review](/documents/review.png)

    - Review does not appear immediately in the pet sitter profile.
    - To be displayed, it must be approved by the admin first.
    - User can delete or edit a review in the user profile in the 'my reviews' section.
...
- ## **As a frequent user:**
  ...
  - ### **User stories**
    ...
    - The profile page contains an extra 'Petsitter profile' section that includes forms to be filled in by the pet sitter, which appear later on on the pet sitter profile page.
      ...
      - In the 'description' category, the pet sitters can describe themselves.
